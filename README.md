...
4. **Spatial + name dedup across sources** (`merge.py`). Two records collapse into one when they are within 100 m of each other and their normalized name-token sets are similar enough — one is a subset of the other, Jaccard similarity ≥ 0.4, or overlap-coefficient ≥ 0.75 with ≥ 2 shared tokens. Priority order is configurable (default `osm > fsq > overture`); the higher-priority record keeps its display name and absorbs the others' names into its alt list.
